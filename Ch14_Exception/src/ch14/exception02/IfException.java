 ...
 * Exception 개념이 도입
 * -> try : 프로그램 로직 실행
 * -> catch : try내에서 로직 실행 중 발생한 모든 에러를 처리하는 곳
 * 
 * Java는 JDK라는 라이브러리를 제공하므로 직접 여러 모듈/클래스/메서드 설계했다.
 * 설계 과정에서 일어날 수 있는 모든 에러에 대해서 '미리 정의된 Exception'을 제공함으로써
 * 개발자가 손쉽게 에러처리를 할 수 있도록 도와준다.
 * 모든 것을 예측할 수 없으므로 개발자가 직접 작성해야 할 때도 있다.
 */
